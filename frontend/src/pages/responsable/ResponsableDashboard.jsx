    ...
    <div className={`app-layout ${sidebarMini ? "sidebar-mini" : ""}`}>
      ...
      <main className="main-content fade-in" style={{ padding: '40px 48px' }}>
        ...
        <section style={{ marginBottom: 40 }}>
          ...
          <p style={{ color: 'var(--text-3)', fontSize: 16 }}>Bienvenue, {user.prenom || 'Responsable'}. Supervision de la promotion en cours.</p>
        </section>
        ...
        >
          <StatCard
            ...
            subtext={`${stats?.nbStagesValides || 0} stages clôturés`}
            trend={14}
            onClick={() => navigate("/responsable/stages")}
          />
          <StatCard
            ...
            trend={5}
            onClick={() => navigate("/responsable/stagiaires")}
          />
          <StatCard
            icon={Target}
            label="Sujets Dispos"
            value={loading ? "..." : stats?.nbSujets}
            color="#f59e0b" delay={0.3}
            subtext="En attente d'affectation"
            trend={-2}
            onClick={() => navigate("/responsable/sujets")}
          />
          <StatCard
            icon={Activity}
            label="Analyse Global"
            value={loading ? "..." : (stats?.moyenneEvaluations?.toFixed(1) || "0.0")}
            color="#10b981" delay={0.4}
            subtext="Qualité de la promotion"
            trend={3}
            onClick={() => navigate("/responsable/analytique")}
          />
        </motion.div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 32, marginBottom: 32 }}>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 40 }}>
              <div>
                <h3 style={{ fontSize: 22, fontWeight: 900, marginBottom: 4 }}>Indicateurs de Performance</h3>
                <p style={{ color: 'var(--text-3)', fontSize: 14 }}>Progression agile et taux de succès des projets</p>
              </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 40 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div style={{ padding: '20px', borderRadius: 20, background: 'var(--bg)', border: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-2)' }}>AVANCEMENT SESSION</span>
                ...
                <div style={{ padding: '20px', borderRadius: 20, background: 'var(--bg)', border: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-2)' }}>VALIDATION FINALE</span>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <div style={{ fontSize: 11, color: 'var(--text-3)', fontWeight: 700 }}>REPARTITION PROJETS</div>
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                { label: 'Affectations Stages', icon: Briefcase, path: '/responsable/stages', color: 'var(--primary)' },
                { label: 'Validation Sujets', icon: ClipboardList, path: '/responsable/sujets', color: 'var(--accent)' },
                { label: 'Mon profil', icon: UserCheck, path: '/responsable/profil', color: 'var(--warning)' },
              ].map((item, i) => (
                <motion.button
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32 }}>
          ...
          >
            <div style={{ padding: '32px 40px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ fontSize: 18, fontWeight: 900 }}>Top Potentiels</h3>
              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--primary)', background: 'var(--primary-lt)', padding: '4px 12px', borderRadius: 20 }}>Promotion</span>
            </div>
                  ...
                  {stats.topStagiaires.map((s, idx) => (
                    <div key={s.stagiaireId} style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-3)' }}>{Math.round(s.tauxAvancement)}% d'avancement</div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ color: 'var(--primary)', fontWeight: 900, fontSize: 16 }}>{s.moyenne.toFixed(1)}</div>
                        <div style={{ fontSize: 10, color: 'var(--text-3)', textTransform: 'uppercase' }}>SCORING</div>
                      </div>
                    </div>
                  ))}
                </div>
              ) : <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-3)' }}>Données en cours de collecte.</div>}
            </div>
          </motion.div>
          ...
          >
            <div style={{ padding: '32px 40px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ fontSize: 18, fontWeight: 900 }}>Charge des Encadrants</h3>
